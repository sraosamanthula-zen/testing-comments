/* 
   This SAS script processes order data to generate detailed reports on parts sales orders. 
   It extracts, transforms, and loads data from various sources, applying business rules to 
   categorize and format the data for reporting purposes. The script fulfills the business 
   requirement of providing insights into order details, including fiscal periods, order 
   statuses, and invoice quantities.
*/
...
SELECT DATEPART(ORDER_DT) AS ORDER_DATE FORMAT = DATE9.,
  /* Fiscal year calculation based on calendar year */
  ...
  /* Extract zonal office name */
  SUBSTR (ZONAL_OFFICE_NAME, 16) AS ZONAL_OFFICE,
  /* Extract area office name */
  ...
  /* Calculate order age if quantity requested is less than total required */
...
/* Create the final report dataset with labeled attributes */
