...
# It allows users to upload two datasets, perform various profiling tasks such as column profiling,
# pattern analysis, duplicate detection, and cross-source matching. The application provides
# visualizations and metrics to help users understand the structure and quality of their data.
...
# with tab_explore:
    ...
    def pattern_percentage(series, pattern):
        """Calculate the percentage of entries in a series that match a given pattern."""
        return series.astype(str).str.fullmatch(pattern).mean() * 100

    def text_length(series):
        """Calculate the minimum, maximum, and average text length of a series."""
    ...
    if not df.empty:
        ...
        def calculate_survivorship(df, status_col, active_value):
            """Calculate the percentage of records that are considered 'surviving'."""
            total_records = len(df)
...
```python
# This Python file is a Streamlit application designed to perform data profiling on two uploaded datasets.
# It provides functionalities such as record and column counts, column profiling, pattern analysis, duplicate detection,
# and cross-source matching. The application aims to assist users in understanding and analyzing their data effectively.
...
# with tab_explore:
    ...
    def text_length(series):
        """Calculate the minimum, maximum, and mean length of text in a series."""
        lengths = series.astype(str).str.len()
        return lengths.min(), lengths.max(), lengths.mean()
    ...
    def column_profiling(df):
        """
        Perform column profiling on the DataFrame.
    ...
    def calculate_survivorship_df(df, status_column="status", active_value="active"):
        """
        Calculate the survivorship rate for a DataFrame based on a status column.

        Parameters:
        df (DataFrame): The DataFrame to calculate survivorship for.
        status_column (str): The column name indicating the status of records.
        active_value (str): The value in the status column that signifies an active record.
    ...
    if not df.empty:
        ...
        def calculate_survivorship(df, status_col, active_value):
            """
            Calculate the survivorship rate for a DataFrame based on a selected status column and active value.

            Parameters:
            df (DataFrame): The DataFrame to calculate survivorship for.
            status_col (str): The column name indicating the status of records.
            active_value (str): The value in the status column that signifies an active record.
    ...
    if df1 and df2:
        ...
        def compute_similarity(row1, row2, cols, weights):
            """
            Compute the similarity score between two rows based on selected columns and weights.
...
```python
# This Python script is a Streamlit application designed for data profiling and analysis. 
# It allows users to upload two datasets and provides functionalities such as record and column counts, 
# column profiling, pattern analysis, duplicate detection, and cross-source matching. 
# The application aims to assist users in understanding the structure and quality of their datasets.
...
# with tab_explore:
    ...
    def text_length(series):
        ...
        Returns:
            tuple: Minimum, maximum, and average text lengths.
        """
        lengths = series.astype(str).str.len()
        return lengths.min(), lengths.max(), lengths.mean()
    ...
    def column_profiling(df):
        ...
        unique values, null counts, and length metrics.

        Args:
            df (pd.DataFrame): The DataFrame to profile.

        Returns:
            pd.DataFrame: A DataFrame containing profiling data for each column.
        """
        profiling_data = []
        for column in df.columns:
            data_type = df[column].dtype
    ...
    def calculate_survivorship_df(df, status_column="status", active_value="active"):
        ...
        Args:
            ...
            status_column (str): The column indicating the status of records.
            active_value (str): The value indicating active records.
    ...
    if not df.empty:
        ...
        def calculate_survivorship(df, status_col, active_value):
            ...
            Args:
                df (pd.DataFrame): The DataFrame to analyze.
                status_col (str): The column indicating the status of records.
                active_value (str): The value indicating active records.
    ...
    if df1 and df2:
        ...
        def compute_similarity(row1, row2, cols, weights):
            ...
            Args:
                ...
                cols (list): The columns to use for comparison.
                weights (dict): The weights for each column.
